...
In compute the distance matrix in R, we use the get_dist and fviz_dist functions. These functions are contained in the factoextra R package.
...
We will now use the K-means clustering method to group the states. The first step is to select the number of clusters.

Let us start with 2 clusters. We also use the nstart function to attempt multiple configurations. This will help identify the optimal configuration.
...
Looking at the results, we can see the cluster means for the two clusters across our 4 variables of Murder, Assault, UrbanPop and Rape. From the results, we can also identify which cluster each state gets assigned to.
...
We can repeat the analysis using different number of clusters and compare the plots to see the differences. here we repeat the analysis with 3, 4, 5 and 6 clusters.
...
We can also compute this using the fviz_nbclust function.
...
The same can also be accomplished using the average silhouette method:
...
We can compute this using the clusGap function which provides the gap statistic and a standard error for the output.
...
As most of the methods suggeested 3 as the most oprimal number of clusters, we perform the final analysis:
